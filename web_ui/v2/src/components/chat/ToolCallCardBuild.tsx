  ...
  return (
    ...
    >
      ...
      <header class="flex items-center gap-2 border-b border-border-subtle px-3 py-2">
        ...
        <span class="truncate text-[12px] font-medium text-text-primary">
          {t(kindLabelKey(props.card.kind))}
        </span>
        <Show when={props.card.file}>
          {(file) => (
            <code class="ml-1 truncate font-mono text-[11px] text-text-tertiary">
              {file()}
            </code>
          )}
        </Show>
        <div class="ml-auto flex items-center gap-2">
          ...
          <Show when={hasBody()}>
            <button
              type="button"
              class="amor-touch flex h-6 w-6 items-center justify-center rounded text-text-tertiary hover:bg-bg-hover focus-visible:outline-2 focus-visible:outline-offset-2"
              aria-expanded={open()}
      ...
      <Show when={hasBody() && open()}>
        <div class="overflow-x-auto px-3 py-2 text-[12px] leading-5">
          ...
          >
            <Suspense
              fallback={
                <pre class="font-mono text-text-tertiary">{renderedBody()}</pre>
              }
            >
              <DiffBlock
                diff={renderedBody()}
      ...
      <Show when={props.card.durationMs != null || props.card.meta}>
        <footer class="flex items-center gap-3 border-t border-border-subtle px-3 py-1.5 text-[11px] text-text-tertiary">
